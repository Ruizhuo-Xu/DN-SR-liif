...
class SRImplicitDownsampled(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.inp_size is None:
            ...
            w_lr = self.inp_size
            w_hr = round(w_lr * s)
            x0 = random.randint(0, hq_img.shape[-2] - w_hr)
            y0 = random.randint(0, hq_img.shape[-1] - w_hr)
            crop_hr_hq = hq_img[:, x0: x0 + w_hr, y0: y0 + w_hr]
            crop_hr_lq = lq_img[:, x0: x0 + w_hr, y0: y0 + w_hr]
            crop_lr_lq = resize_fn(crop_hr_lq, w_lr)
            crop_hr = crop_hr_hq
            crop_lr = crop_lr_lq
